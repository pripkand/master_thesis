...
def anharmonic_thermal_energy(beta, N=140, omega=2.0):
    a = np.zeros((N, N))
    for n in range(1, N):
        a[n-1, n] = np.sqrt(n)
    adag = a.T
    x = (a + adag)/np.sqrt(2*omega)
    p = 1j*np.sqrt(omega/2)*(adag - a)
    H = (p @ p) + (x @ x @ x @ x)
    H = (H + H.conj().T)/2
    E, _ = la.eigh(H)
    weights = np.exp(-beta * E.real)
    Z = np.sum(weights)
    return np.sum(E.real * weights)/Z