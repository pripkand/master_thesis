...
class PsdHermitian(MutableMapping):
    ...
    def __str__(self):
        return f"Hermitian Matrix {self._name} with dimension: {self.size} blocks {self._nblock} block structure {self._blockstruct} and number of variables {self.mdim}"
    ...
    def __neg__(self):
        return PsdHermitian({key:-value for key,value in self._matrix.items()},{key:False for key in self._variables},name= "-"+self._name,nblock= self._nblock,blockstruct= self._blockstruct)
...
# Basically there. Next is to try and run for a single temperature the code for the harmonic oscillator from my
# mathematica exports. If that is good, I just need to write an extra method that takes the solutions from the file and
# returns them accordingly. Possibly make it both a class method and an actual method.